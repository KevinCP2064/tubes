...
public class ServiceDaoImpl implements DaoService<Service> {
    ...
    public List<Service> fetchAll() throws SQLException, ClassNotFoundException {
        ...
        try (Connection connection = MySQLConnection.createConnection()) {
            String query = "SELECT s.id_nota,s.tanggal,s.jns_service,s.keterangan,s.harga,k.no_plat,u.id,u.nama,r.id_role,r.role FROM service s JOIN kendaraan k ON s.kendaraan_no_plat=k.no_plat" +
                    "JOIN user u ON k.user_id=u.id JOIN role r ON u.role_id_role=r.id_role WHERE role='owner'";
